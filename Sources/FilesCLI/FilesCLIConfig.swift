...
struct FilesCLIConfig: Sendable {
    ...
    /// - Throws: `FilesCLIConfigError` if JSON file is malformed or missing required fields
    init?(configPath: String?) async throws {
        let path = configPath ?? Self.defaultFileName
        guard FileManager.default.fileExists(atPath: path) else {
            if configPath != nil {
                throw FilesCLIConfigError.missingFile(path: path)
            }
            return nil
        }
    ...
    init(configFilePath: FilePath) async throws {
        ...
        guard configFileManager.fileExists(atPath: configPathString) else {
            throw FilesCLIConfigError.missingFile(path: configPathString)
        }
        ...
        do {
            ...
            throw FilesCLIConfigError.invalidJSON(
                path: configPathString,
                reason: decodingError.localizedDescription
            )
        } catch {
            throw FilesCLIConfigError.readFailed(
                path: configPathString,
...
enum FilesCLIConfigError: Error {
    case missingFile(path: String)
    case invalidJSON(path: String, reason: String)
    case readFailed(path: String, reason: String)
}

extension FilesCLIConfigError: LocalizedError {
