...
Inheritance chains are followed through the analyzed source. A base class that lives outside the source — `class ProductCell: UICollectionViewCell` — is still resolved: scout extracts the real class hierarchy from the Xcode SDK with `swift-symbolgraph-extract` for the modules the source imports. There is no hardcoded UIKit mapping, so `UICollectionViewCell`, `UITableViewCell` and `UIControl` subclasses all land under `UIView`.
